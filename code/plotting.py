...
def plot_marginal_completeness_2d(x_grid, y_grid, marg_comp, xlabel, ylabel, contours=False):
...
def plot_det_eff_2d(grids, comp, label):
    ...
    plot_marginal_completeness_2d(period_grid[:, :, 0], rp_grid[:, :, 0], comp_moc, "Period", "$R_p \, [R_\oplus]$")
    plot_marginal_completeness_2d(period_grid[:, 0, :], c_grid[:, 0, :], comp_mor, "Period", label)
    plot_marginal_completeness_2d(rp_grid[0, :, :], c_grid[0, :, :], comp_mop, "$R_p \, [R_\oplus]$", label)


def plot_marginal_completeness_1d(x_grid, marg_comp, xlabel, ylabel, contours=False):
...
def plot_det_eff_1d(grids, comp, var_names):
    ...
    # Plot the three combinations
    plot_marginal_completeness_1d(period_grid[:, 0, 0], comp_p, var_names[0], "Completeness")
    plot_marginal_completeness_1d(rp_grid[0, :, 0], comp_r, var_names[1], "Completeness")
    plot_marginal_completeness_1d(c_grid[0, 0, :], comp_c, var_names[2], "Completeness")
...
def plot_occ_rates(kois, grids, comp, var_names, bins):
    ...
    plot_marginal_completeness_1d(period_grid[:-1, 0, 0], hist_p/comp_p[:-1], var_names[0], "Occurrence rate")

    hist_r, _ = np.histogram(kois["{}".format(var_names[1])], bins[1])
    plot_marginal_completeness_1d(rp_grid[0, :-1, 0], hist_r/comp_r[:-1], var_names[1], "Occurrence rate")

    hist_c, _ = np.histogram(kois["{}".format(var_names[2])], bins[2])
    plot_marginal_completeness_1d(c_grid[0, 0, :-1], hist_c/comp_c[:-1], var_names[2], "Occurrence rate")
